...
def train(args, train_dataset, eval_dataset, model, meters, recover_args, logger):
    ...
    for epoch in range(start_epoch, int(args.num_train_epochs)):
        ...
        for step, batch in enumerate(tbar):
            ...
            dialog_img_feat = batch[6].to(args.device, non_blocking=True)
            persona_img_feats = batch[7].to(args.device, non_blocking=True)
            dialog_img_mask = batch[8].to(args.device, non_blocking=True)
            persona_img_mask = batch[9].to(args.device, non_blocking=True)
...
def evaluate(args, model, eval_dataset, mode, logger, prefix=''):
    ...
    for batch in tqdm(test_dataloader, ncols=70):
        ...
        dialog_img_feat = batch[6].to(args.device, non_blocking=True)
        persona_img_feats = batch[7].to(args.device, non_blocking=True)
        labels = batch[8].to(args.device, non_blocking=True)
        example_indices = batch[9].to(args.device, non_blocking=True)
        dialog_img_mask = batch[10].to(args.device, non_blocking=True)
        persona_img_mask = batch[11].to(args.device, non_blocking=True)
        ...
        with torch.no_grad():
            ...
            results_dict['example_indices'].append(example_indices.cpu().detach().numpy())
